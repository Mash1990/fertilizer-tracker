...
class FabCar extends Contract {
  async initLedger(ctx) {
    ...
    for (const fertilizer of fertilizers) {
      ...
      await ctx.stub.putState(
        ...
        Buffer.from(JSON.stringify(fertilizer))
      );
  ...
  async acceptTransfer(ctx, fertilizerId, acceptingParty) {
    console.info("============= START : Accept Transfer ===========");
    ...
    // Check if the transfer is pending and the accepting party is correct
    if (
      fertilizer.transferStatus === "pending" &&
      fertilizer.pendingOwner === acceptingParty
    ) {
      fertilizer.owner = acceptingParty;
      fertilizer.transferStatus = "accepted";
      await ctx.stub.putState(
        fertilizerId,
        Buffer.from(JSON.stringify(fertilizer))
      );
      console.info(
        `Transfer accepted for fertilizer ${fertilizerId} by ${acceptingParty}`
      );
    } else {
      throw new Error(
        `Transfer is not pending or accepting party is not correct for ${fertilizerId}`
      );
    }

    console.info("============= END : Accept Transfer ===========");
  }
  ...
  async changeFertilizerOwner(ctx, fertilizerId, newOwner) {
    ...
    // Set the transfer as pending and specify the pending new owner
    fertilizer.transferStatus = "pending";
    fertilizer.pendingOwner = newOwner;
    ...
    console.info(
      `Owner change requested for fertilizer ${fertilizerId} to ${newOwner}`
    );
